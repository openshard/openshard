...
This includes the structured `verification` block
(`history/verification.py`): `status`, `source`, `observation_mode`, check
counts and names, timestamps, `artifact_sha` and `complete` /
`incomplete_reasons`, exactly as `history --json` prints it. The flat
summary fields are sent alongside it:
...
the evidence source, e.g. `[directly_observed]` (a hook saw the check
command invoked; outcome not observed), `[agent_reported]` or
`[independently_verified @ abc123f]`.

`task_title` (the concise display title from `history/task_title.py`) is
sent too, next to `task_full`, which still carries the original task text
unchanged.
