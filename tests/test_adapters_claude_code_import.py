...
class TestTaskIdAttachment(unittest.TestCase):
    ...
    def test_no_task_id_argument_means_no_task_id_field(self):
        entry = build_claude_code_import_entry(
            "Fix the bug", model=None, notes_file=None, repo_path=self.repo,
        )
    ...
    def test_explicit_task_id_is_preserved_exactly(self):
        ...
        tid = new_task_id()
        entry = build_claude_code_import_entry(
            "Fix the bug", model=None, notes_file=None, repo_path=self.repo, task_id=tid,
        )
