...
yet. Nothing is added, renamed or filled in here: a key the Platform
contract does not define is the Platform's to reject, and that rejection is
recorded locally rather than papered over.

``WITHHELD_RECEIPT_KEYS`` holds additive fields a deployed Platform may not
accept yet. The contract is strict and a rejection is never retried, so a
new key is sent only once the Platform contract defines it. Withheld today:

- ``verification`` -- summarised into the contract's existing
  ``verification_status`` / ``verification_reason`` fields.
- ``task_title`` -- the Platform derives the same deterministic title from
...
# once the Platform contract defines it. ``task_title`` is display-only and
# re-derivable by the Platform from ``task_full``.
WITHHELD_RECEIPT_KEYS: frozenset[str] = frozenset({"verification", "task_title"})
