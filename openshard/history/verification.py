...
``source``
    ...
    ``agent_reported`` (the agent said so, or relayed it through its hook
    stream), ``directly_observed`` (OpenShard ran the check and read its
    exit code), ``git_verified`` (bound to a commit git confirmed),
...
MODE_HOOK_TOOL_EVENT = "hook_tool_event"  # agent hook relayed that a check command ran
...
def _from_hook_record(entry: dict, capture: dict, attempted: bool) -> VerificationEvidence:
    ...
    ev = VerificationEvidence(source=SOURCE_AGENT_REPORTED, observation_mode=MODE_HOOK_TOOL_EVENT)
